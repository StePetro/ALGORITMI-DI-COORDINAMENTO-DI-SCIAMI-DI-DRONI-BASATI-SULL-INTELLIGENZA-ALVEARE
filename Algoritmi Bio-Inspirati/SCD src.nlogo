__includes [
  ...
  ;; Nuovi algoritmi
  "include/exploration_algorithm.nls"
  "include/ABC-RR.nls"
  "include/execution_procedures.nls"
  "include/recruitment_procedures.nls"
  "include/pso.nls"
  "include/FTS-RR.nls"
...
to go
  ...
  if (percentageTgtsFound > 95 and showstat < 1)[  ;;vengono stampati sul terminale di netlogo i tick relativi alla simulazione e stoppata la stessa al 100% di esecuzione
    show "Trovato il 95% dei target al tick:"
    show ticks
    set showstat showstat + 1
  ]

  if (percentageTargetsExecuted > 95 and showstat < 2)[
    show "Eseguito il 95% dei target al tick:"
    show ticks
    set showstat showstat + 1
  ]

  if (percentageTgtsFound = 100 and showstat < 3)[
    show "Trovato il 100% dei target al tick:"
    show ticks
    set showstat showstat + 1
  ]

  if (percentageTargetsExecuted = 100)[
    show "Eseguito il 100% dei target al tick:"
    show ticks
    stop
  ]
...
;; Fase 1: esplorazione da parte dei droni nello stato EXPLORER
;; Fase 2: reclutamento di droni da parte dei coordinatori
;; Fase 3: sviluppo del task (da realizzare)
to droneLogic

  ;; ALGORITMO DI ESPLORAZIONE (per testare solo l'esplorazione: andare in "exploration_algorithm" e commentare il passaggio del drone in coordinatore)
  algorithm_1

  ;; ALGORITMI DI RECLUTAMENTO (commentare i due che non servono)
...
"" ""
PENS
"found" 1.0 0 -2064490 true "" "plotxy ticks percentageTgtsFound"
"executed" 1.0 0 -16448764 true "" "plotxy ticks percentageTargetsExecuted"
...
"" ""
PENS
"EXPLORER" 1.0 0 -10899396 true "" "plotxy ticks (count (drones with [drone.state = \"EXPLORER\"]))"
"RECRUITED" 1.0 0 -13345367 true "" "plotxy ticks (count (drones with [drone.state = \"RECRUITED\"]))"
"WAITING" 1.0 0 -6459832 true "" "plotxy ticks (count (drones with [drone.state = \"WAITING\"]))"
"EXECUTION" 1.0 0 -8630108 true "" "plotxy ticks (count (drones with [drone.state = \"EXECUTION\"]))"
...
334
EXPLORER
count drones with [ drone.state = \"EXPLORER\"]
17
1
14
...
335
RECRUITED
count drones with [drone.state = \"RECRUITED\"]
17
1
14
...
400
WAITING
count drones with [drone.state = \"WAITING\"]
17
1
14
...
400
EXECUTION
count drones with [drone.state = \"EXECUTION\"]
17
1
14
